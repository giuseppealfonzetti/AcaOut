check_grades <- function(MAT){
  ...
  if(!all(gtab==gtab[1])){
    stop("Some grades have not been observed on some exams. Investigate with `apply(MAT, MARGIN=2, FUN=table)`.")
  }
...
check_todo <- function(MAT){
  ...
  if(any(colSums(MAT)==0)){
    stop("Some exams are not included in any study plan.\n Investigate with `colSums(MAT)`.")
  }
...
check_times <- function(MAT){
  ...
  if(any(ttab==0)){
    stop("Some exams have not been passed by any student yet. Investigate with `apply(MAT, MARGIN = 2, function(x) length(x[is.finite(x)]))`.")
  }
...
check_data <- function(GRADES, TIMES, TODO, OUTCOME, X, FIRST_YEAR, LAST_YEAR, LAST_EXAM_YEAR, MAX_TIME,
  ...
  if(!all(which(is.na(GRADES))==which(is.na(TIMES)))){
    stop("For one or more exam, some observations have observed grade but missing time or viceversa.")
  }

  if(!all(which(!is.na(GRADES)) %in% which(TODO) )){
    stop("Some observations have observed grade on one or more exams not included in the study plan.")
  }
