...
fitCCR <- crirt::fit_CCR(DATA=dat, PAR_START = grtcEM$fit$par, LATMAT = compute_map(grtcEM, TIDY = FALSE))
par_start <- grtcEM$fit$par
par_start[(dat$dim_irt+3):(dat$dim_irt+dat$dim_cr+2)] <- fitCCR$fit$par
...
```{r fullSE}
fullSE <- compute_stderr(fullBFGS, TIDY = TRUE)
      ...
      if(year==1){
        ...
        prod(sapply(1:(year-1), function(yr){
          ...
          haz_g <- pL*cpp_hazard(
            ...
            YEAR= yr,
            THETA=theta_chosen,
            COVARIATES = c(as.numeric(colMeans(fullBFGS$data$X)), ability, speed),
            YB=fullBFGS$data$yb,
            LOGFLAG=FALSE)$prob
